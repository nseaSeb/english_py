...
phrases = [
    ...
    "I'll do it later, I need a break.",
    # Autres prisent ici et là
    "I'm going to write down my tasks for tomorrow before I shut down my laptop",
    "It's a shame that personal discussions waste so much of our time.",
    "Maybe we could organize a quick online meeting next week?",
    "I have to finish this project by Friday.",
    # tech
    "I'm going to start by reproducing the issue to understand the root cause.",
]
...
with gr.Blocks() as demo:
    ...
    with gr.Tab("🎙️ Speech Recognition"):
        gr.Markdown(
            "Speak into the microphone. The app will recognize what you said and reply aloud."
        )
        mic_input = gr.Audio(
            sources=["microphone"], type="filepath", label="Speak here"
        )
    ...
    with gr.Tab("🎲 Random Sentences"):
        ...
        with gr.Row():
            random_repeat_button = gr.Button(
                "🔁 Répéter la phrase", variant="secondary"
            )
    ...
    with gr.Tab("✍️ Custom Text"):
        ...
        custom_input = gr.Textbox(
            label="Your text", placeholder="Type your sentence here...", lines=3
        )
        custom_output = gr.Textbox(label="Text played", interactive=False)
        custom_button = gr.Button("🔊 Speak This Text")
        custom_button.click(
            fn=say_custom_text, inputs=custom_input, outputs=custom_output
        )
        with gr.Row():
